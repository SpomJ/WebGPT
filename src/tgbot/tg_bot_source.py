...
alia = AutoModelForCausalLM.from_pretrained("checkpoint-79000/content/drive/MyDrive/gpt-oasst/checkpoint-79000").to(
    'cuda')

alia_tokenizer = AutoTokenizer.from_pretrained("checkpoint-79000/content/drive/MyDrive/gpt-oasst/checkpoint-79000",
                                               device="cuda:0")
...
def google_ans(question):
    try:
        search_results = iter(list(googlesearch.search(question, num_results=1)))
    except requests.exceptions.HTTPError:
        search_results = iter([search_url(question)])
    if search_results:
        website_url = next(search_results)
        url = website_url
...
def get_all_res(question):
    ...
    for i, segment in enumerate(text_segments):
        inputs = tokenizer(question, segment, return_tensors="pt", truncation=True, max_length=512,
    ...
    alia_outputs = alia.generate(**input_ids, max_length=512, repetition_penalty=10., encoder_repetition_penalty=10.,
                                 eos_token_id=alia_tokenizer(S_END)['input_ids'][0])
